			...
			<div class="card ">
				...
				<div class="card-body">
					<div class="mb-3">
						...
						<input type="text" class="form-control form-control-border" id="user_name" value="<?= $user_name ?>" required>
					</div>
					<div class="mb-3">
						<label for="user_password" class="form-label">Kullanıcı Parolası</label>
						<input type="text" class="form-control form-control-border" id="user_password">
					</div>
					<div class="mb-3">
						...
						<select class="custom-select form-control-border rounded-0" id="user_group" name="status" aria-label="Default select example">
							...
							foreach ($groups as $group) { ?>
								...
								<option value="<?=$group->id?>" <?=($group->id==$user_group) ? "selected" : null?> ><?=$group->group_name?></option>
							<?php } ?>
					...
					<div class="mb-3 col-lg-4 col-sm-12">
						...
						<select class="custom-select form-control-border rounded-0" id="status" name="status" aria-label="Default select example">
							<option value="1" <?=($user_status==1) ? "selected" : null?> >Aktif</option>
							<option value="0" <?=($user_status==0) ? "selected" : null?> >Pasif</option>
						</select>
					</div>
				</div>
...
<script>
	...
	function submitLink() {
		...
		if (user_name != "" && user_group != "" && status != "") {
			$.ajax({
				type: "POST",
